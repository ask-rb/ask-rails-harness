...
class MinitestJsonReporterTest < Minitest::Test
  ...
  def test_report_writes_counts_and_test_entries
    reporter = Ask::Rails::Harness::MinitestJsonReporter.new(@json_path)
  ...
  def test_report_writes_nothing_without_path
    reporter = Ask::Rails::Harness::MinitestJsonReporter.new(nil)
    reporter.record(passing_result)
    reporter.report
  ...
  # so minitest auto-discovers lib/minitest/ask_rails_harness_plugin.rb and
  # the plugin registers the reporter via ASK_TEST_JSON_PATH — exactly what
  # happens when an app runs `bin/rails test` through the harness.
