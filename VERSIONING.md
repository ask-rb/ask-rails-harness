...
- **0.x.MINOR** — new functionality, *and* any breaking change (removing or
  renaming public tools/API, changed defaults or behavior). Pre-1.0, MINOR
...
Patch numbers advance by exactly one per release: `0.4.4` → `0.4.5` →
`0.4.6`. Never skip or jump patch numbers (`0.4.4` → `0.4.7` is wrong), even
...
Every `ask-*` gem — ask-rails-harness included — and **yamine** is released
...
gemchain guard ask-rails-harness
gemchain update ask-rails-harness <new-version> --dry-run
gemchain update ask-rails-harness <new-version> --test-only
gemchain update ask-rails-harness <new-version>
```
...
ask-rails-harness depends on `ask-ruby-harness` and `ask-auth`. Releasing
either is never a single-gem event: one `gemchain update <dep> <ver>` from
the workspace root rewrites this gem's gemspec constraint, re-runs this
suite, bumps ask-rails-harness one patch, commits, publishes, tags, and
pushes — in topological order. The cascade also runs in the other direction:
`ask-rails-harness-mcp` depends on this gem and is released when this gem
changes. `gemchain guard` shows the full blast radius; never hand-edit a
...
3. **Build** — `gem build ask-rails-harness.gemspec` succeeds (gemchain does
...
4. **Changelog** — `## [Unreleased]` entries moved under a new
   ...
   opened above it.
5. **Version** — bumped in `lib/ask/rails/harness/version.rb` by gemchain,
   not by hand.
...
| `lib/ask/rails/harness/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
Starting from `0.4.4`, released via `gemchain update ask-rails-harness
<ver>` from the workspace root:

| Change | Next version | Why |
|---|---|---|
| Fix a route-inspector crash | `0.4.5` | pre-1.0 fix ⇒ sequential PATCH |
| Two bug fixes land together | `0.4.6` (one release, not `0.4.7`) | one step, however many fixes |
| Add a new harness tool | `0.5.0` | new feature ⇒ pre-1.0 MINOR |
| Remove a public tool (breaking) | `0.5.0` | breaking ⇒ pre-1.0 MINOR |
| Declare the API stable | `1.0.0` | first stable major |
| `ask-ruby-harness` releases | cascade via `gemchain update ask-ruby-harness <ver>` | this gem's constraint is rewritten and it is re-tested, bumped one patch, published — never by hand |
